...
async def change_lang(update, context):
    ...
    if txt not in ['RU -> EN', 'EN -> RU']:
        if not context.user_data.get('way') or context.user_data['way'] == -1:
            await update.message.reply_text('Для начала выберите направление перевода.')
            return
            ...
            response = requests.post(
                ...
                json=body, headers=headers)
            res = response.json()['translations'][0]['text']
        await update.message.reply_text(f'Результат перевода:\n{res}')
    else:
        context.user_data['way'] = txt
        await update.message.reply_text(f'Направление перевода обновлено на: {txt}.')
